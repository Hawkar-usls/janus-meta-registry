# -*- coding: utf-8 -*-
"""
MCT Grid Search v4 — Physarum Maze (слизевик в лабиринте) - УЛЬТРАСЛОЖНАЯ ВЕРСИЯ
...
- Параметры усилены до предела: 60 узлов, разреженный граф, мало ресурсов, высокий шум.
"""
...
# ==============================================================================
# Конфигурация эксперимента (УЛЬТРАСЛОЖНАЯ)
# ==============================================================================
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
N_SEEDS = 10               # число сидов на конфигурацию
STEPS = 20000              # шагов обучения (увеличено, так как сложнее)
EVAL_EVERY = 2000          # частота оценки
LEARNING_RATE = 0.001      # Adam

# Размеры выборки (уменьшены, чтобы усилить переобучение)
TRAIN_SIZE = 3000
VAL_SIZE = 600

# Параметры среды (максимальная сложность)
N_NODES = 60                # количество узлов в графе
EDGE_PROB = 0.08            # разреженный граф (чтобы было меньше путей)
INIT_RESOURCE = 1.5         # начальное количество ресурса в каждом узле (мало)
RESOURCE_REGEN = 0.02       # очень медленное восстановление
RESOURCE_THRESHOLD = 1.0    # порог, при котором ресурс считается доступным (только целые)
EXPLOIT_STRENGTH = 1.2      # μ – слабая нелинейность (чуть больше 1)
NOISE_LEVEL = 0.9           # очень высокий уровень шума (исследование)

# Модель (немного увеличим ёмкость, чтобы она могла выучить сложную структуру)
N_EMBD = 96
N_LAYER = 3
N_HEAD = 6
SEQ_LEN = 256               # длинные последовательности
...
# ==============================================================================
# 2. МОДЕЛЬ (увеличенная ёмкость)
# ==============================================================================
class HierarchicalTransformer(nn.Module):
    def __init__(self, vocab_size, n_embd=96, n_head=6, n_layer=3, block_size=256):
        super().__init__()
    ...
    def forward(self, idx):
        seq_len = idx.size(0)
        # Проверка, что seq_len не превышает block_size
        if seq_len > self.block_size:
            # обрезаем последовательность
...
def run_grid_search(model_sizes, gain_vals, temp_vals,
                    ...
                    train_size=3000, val_size=600, device='cpu'):
    results = []
...
def sanity_run():
    ...
    print("Генерация датасета (Physarum Maze ULTRA)...")
    train_data = make_physarum_dataset(
        num_sequences=TRAIN_SIZE, seq_len=SEQ_LEN,
        n_nodes=N_NODES, edge_prob=EDGE_PROB,
        init_resource=INIT_RESOURCE, resource_regen=RESOURCE_REGEN,
    ...
    history, final = train_model(
        ...
        steps=10000, lr=LEARNING_RATE, eval_every=2000, device=DEVICE
    )
    ...
    if gap > 1.0:
        print("✅ Ландшафт очень сложный, gap отличный.")
        return True
    elif gap > 0.5:
...
if __name__ == "__main__":
    ...
    model_sizes = [N_EMBD]  # фиксируем размер, можно добавить другие
