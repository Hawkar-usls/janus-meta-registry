...
Human Rights Watch has documented Russian network manipulation, filtering and isolation/shutdown capability. Russian authorities have also publicly operated a mobile **deny-except-allowlist** model in which broad mobile Internet access is restricted while selected approved services remain available.
...
There is a historical routing precedent. Cloudflare independently measured occupied Ukrainian networks whose upstream connectivity shifted onto Russian networks during the war. This demonstrates that the information-access and communications environment can change when the path itself is moved into a different control jurisdiction.
...
The following composite would be more meaningful than any single censorship report:
...
- and device/vendor/control-plane evidence before attributing the event to any foreign state.
...
CISA's emergency communications guidance emphasizes resilient and secure communications, route diversity, redundancy and alternative paths. The concern described here is fundamentally a **communications resilience and dependency problem**, not a request for CISA to assess propaganda content.
...
**Routing note:** This draft is an early-warning/research submission, not a claim of a current incident, and is not yet approved for sending.
