...
def adaptive_history_copy():
    def p(mode):
        hit = total = 0
        if mode in ('REAL','BLIND','FRESHNESS_ONLY_SHUFFLE'):
            for t1 in (0,1):
                total += 1
                t2=t1; b1=0; b2=t1
    ...
    return {
        ...
        'exact_state_count':{'REAL':2,'REWIRED_OR_NULL':4,'ALT_PREDECESSOR':4},
    }

def protocol_order(edge_id, phases, required_order, freshness_orthogonal=True):
    perms=list(permutations(phases))
    def valid(order):
        pos={x:i for i,x in enumerate(order)}
        return all(pos[required_order[i]] < pos[required_order[i+1]] for i in range(len(required_order)-1))
    null_valid=sum(valid(p) for p in perms)
    null_rate=Fraction(null_valid, len(perms))
    raw={
        'REAL':Fraction(1),
        'BLIND':Fraction(1),
        'TEMPORAL_REWIRED':Fraction(0),
        'MATCHED_NULL':null_rate,
        'FRESHNESS_ONLY_SHUFFLE':Fraction(1 if freshness_orthogonal else 0),
        'ALTERNATIVE_PREDECESSOR':Fraction(0),
    }
...
benchmarks.append({
    ...
    'frozen_model':'T1~Bernoulli(1/2); B1=0. REAL: T2=T1 and B2=T1. REWIRED/NULL: T2 independent fair while B2=T1. ALT: T2=T1 while B2=U independent fair. FRESHNESS control changes only an orthogonal metadata bit.',
    'result':b1,
    'classification':classify(b1['normalized_relation_score'])
})

b2=protocol_order('B2', ['PRE_RETURN_FREEZE','EXTERNAL_APPEND_ONLY_ANCHOR','TARGET_GENERATION'], ['PRE_RETURN_FREEZE','EXTERNAL_APPEND_ONLY_ANCHOR','TARGET_GENERATION'], True)
benchmarks.append({
    ...
    'frozen_model':'Minimal critical core has exactly three phases. REAL uses required order. BLIND renames phases only. REWIRED violates phase order. NULL is the complete 3! permutation set. FRESHNESS changes an orthogonal freshness tag without moving phases. ALT replaces the required immediate predecessor relation with a matched wrong predecessor.',
    'result':b2,
    'classification':classify(b2['normalized_relation_score'])
})

b3=protocol_order('B3', ['ROUTER_FREEZE','EXTERNAL_COMMITMENT','PUBLIC_CASE_REVEAL'], ['ROUTER_FREEZE','EXTERNAL_COMMITMENT','PUBLIC_CASE_REVEAL'], True)
benchmarks.append({
    ...
    'frozen_model':'Minimal ceremony core is phases ROUTER_FREEZE, EXTERNAL_COMMITMENT, PUBLIC_CASE_REVEAL. NULL enumerates all 3! phase orders. FRESHNESS changes an orthogonal timestamp/freshness tag only. ALT substitutes a matched wrong predecessor for EXTERNAL_COMMITMENT.',
    'result':b3,
    'classification':classify(b3['normalized_relation_score'])
})

neg=protocol_order('NC1', ['AUTHENTIC_CHECKPOINT','EXTERNAL_FRESHNESS_ROOT','CURRENT_ADMISSIBILITY'], ['AUTHENTIC_CHECKPOINT','EXTERNAL_FRESHNESS_ROOT','CURRENT_ADMISSIBILITY'], False)
benchmarks.append({
    ...
    'frozen_model':'Same 3-phase order benchmark, except FRESHNESS_ONLY_SHUFFLE deliberately breaks the freshness-root/currentness association because freshness is the mechanism rather than an orthogonal confounder.',
    'result':neg,
    'classification':classify(neg['normalized_relation_score'])
})

out={
    'schema':'janus.connection.six_state_benchmark.v1',
    'benchmark_id':'JANUS-CONNECTION-SIX-STATE-BENCHMARK-2026-08-14-V1.0',
    'execution_mode':'EXACT_ENUMERATION_FORMAL_RELATION_CORES',
    ...
    'summary':{
        ...
        'claim_ceiling':'Executed exact benchmark of frozen formal relation cores. This is not yet a full raw-JSON field-permutation benchmark, external replication, causal proof, or scientific novelty claim.'
    }
}
