...
SESSION.headers.update({"User-Agent": "JANUS-JPFM-3A-cross-event-atlas/1.0"})
...
def fetch_gfz_extended():
    ...
    if d.isn.isna().any() or d.f107_adj.isna().any():
        bad = d[d.isn.isna() | d.f107_adj.isna()][["date_obj","isn","f107_adj"]].head(10).to_dict("records")
        raise RuntimeError(f"GFZ solar fields incomplete in study window: {bad}")
    f107_q90 = float(d.f107_adj.quantile(.90, interpolation="linear"))
    ...
    return d, {
        "source_url": GFZ_URL, "source_sha256": sha_bytes(b), "rows": len(d),
        "f107_adj_q90": f107_q90, "isn_q90": isn_q90,
        "event_day_counts": {c:int(d[c].sum()) for c in ["D1_SOLAR_F107_HIGH","D2_SUNSPOT_HIGH","D3_GEOMAG_STORM","D4_GEOMAG_SEVERE_SENSITIVITY"]}
    }
...
def main():
    ...
    admission=json.loads(ADMISSION.read_text(encoding="utf-8")); star_counts,star_meta=load_star_freeze(); star_days=set(star_counts)
    ...
    gfz,gfz_meta=fetch_gfz_extended(); gs=gfz[["date_obj","daily_Ap","max_kp","isn","f107_adj"]].copy(); gs["date_obs"]=gs.date_obj.astype(str)
    ...
    event_sets={
        "D1_SOLAR_F107_HIGH":{d for d,r in gfz_by_date.items() if r.D1_SOLAR_F107_HIGH},
    ...
    for arm,days in event_sets.items():
        ...
        lags=range(-7,8) if arm in {"D1_SOLAR_F107_HIGH","D2_SUNSPOT_HIGH"} else (range(-4,5) if arm.startswith("D3") or arm.startswith("D4") else range(-3,4))
    ...
    for arm in ["D6_LUNAR_DARK","D7_LUNAR_BRIGHT"]:
        fit=fit_event_nb(nights,arm,False,True); poss[arm]={"event_days_on_observed_nights":int(nights[arm].sum()),"fit":fit,"bluebook_layer":"DEFERRED_NO_FULL_CALENDAR_USNO_FREEZE"}; poss_p[arm]=fit.get("p_two_sided")
    poss_q=bh(poss_p)
    for arm,q in poss_q.items(): poss[arm]["bh_q"]=q
    print("[6/7] frozen external families on Blue Book STARLIKE layer",flush=True)
    blue={}; blue_p={}
    for i,arm in enumerate(["D1_SOLAR_F107_HIGH","D2_SUNSPOT_HIGH","D3_GEOMAG_STORM","D5_MAJOR_METEOR_SHOWER_PROXY"]):
    ...
    result={
      ...
      "bindings":{"admission":str(ADMISSION),"admission_sha256":sha_file(ADMISSION),"bluebook_starlike_freeze":str(STAR_FREEZE),"bluebook_starlike_meta":star_meta,
